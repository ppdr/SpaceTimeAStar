...
public:
    Map(const int width, const int height, const std::vector<Location> static_obstacles, const std::vector<State> vertex_constraints, const std::vector<std::pair<State, State>> edge_constraints) : width(width), height(height), static_obstacles(static_obstacles), vertex_constraints(vertex_constraints), edge_constraints(edge_constraints) {};
